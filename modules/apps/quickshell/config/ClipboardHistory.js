function normalizeEntry(value) {
  ...
  if (type === "text") {
    ...
    if (value.id !== undefined && value.id !== null && String(value.id))
      textEntry.id = String(value.id)
    return textEntry
  }
...
// The picker only searches and renders a prefix. Actions use the original
// history index to read the complete entry, so this does not lose data.
...
function cappedEntry(entry) {
  ...
  // Cut on a line break so a file URI never becomes a truncated bogus path.
