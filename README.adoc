...
`fos` is the Froidmont Operating System command center. It uses the configured
NixOS checkout from `programs.nh.flake` and can be run from any directory.
...
fos boot
fos update
fos update nixpkgs home-manager
fos generations
fos rollback
fos rollback 42
----

Only `fos build` accepts another host. Activation and rollback commands always
target the local machine. `fos update` updates the selected inputs and builds
the local configuration without activating it.
